...
import flu.kitten.adorablearmory.util.ColourARGB;
...
public class HaloBakedModel extends WrappedItemModel {
    ...
    static BakedQuad generateHaloQuad(final TextureAtlasSprite sprite, final int size, final int color) {
        ...
        putVertex(quad.vertices[0], maxX, maxY, maxU, minV);
        putVertex(quad.vertices[1], minX, maxY, minU, minV);
        putVertex(quad.vertices[2], minX, minY, minU, maxV);
        putVertex(quad.vertices[3], maxX, minY, maxU, maxV);
        for (int i = 0; i < 4; ++i) {
    ...
    static void putVertex(final Quad.Vertex vx, final double x, final double y, final double u, final double v) {
        vx.vec[0] = (float) x;vx.vec[1] = (float) y;vx.vec[2] = (float) 0.0;vx.uv[0] = (float) u;vx.uv[1] = (float) v;
    }
    ...
    public void renderItem(ItemStack stack, ItemDisplayContext context, PoseStack poseStack, MultiBufferSource source, int packedLight, int packedOverlay) {
        if (context == ItemDisplayContext.GUI) {
            renderHalo(stack, poseStack, source, packedLight, packedOverlay);
            this.renderWrapped(stack, poseStack, source, packedLight, packedOverlay, true);
            if (this.pulse) {
                renderPulse(stack, poseStack, source, packedLight, packedOverlay);
            }
        } else {
            this.renderWrapped(stack, poseStack, source, packedLight, packedOverlay, true);
        }
    ...
    private void renderHalo(ItemStack stack, PoseStack pStack, MultiBufferSource source, int packedLight, int packedOverlay) {
        ...
        if (this.pulse) {
            renderPulse(stack, pStack, source, packedLight, packedOverlay);
        }
    }

    private void renderPulse(ItemStack stack, PoseStack pStack, MultiBufferSource source, int packedLight, int packedOverlay) {
        pStack.pushPose();
        try {
            double pulseIntensity = getPulseIntensityForContext(ItemDisplayContext.GUI);
