...
import flu.kitten.adorablearmory.entity.boss.animations.ScarletLoraAlysiaAnimation;
...
public class ScarletLoraAlysiaModel<T extends ScarletLoraAlysia> extends HierarchicalModel<T> implements ArmedModel {
    ...
    public void setupAnim(@NotNull T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose); // 重置姿势
        ...
        if (entity.isUsingItem()) {
            ...
            if (usingMainHand == mainRight) {
                this.poseRightArm(entity);
            } else {
                this.poseLeftArm(entity);
            }
        } else {
            boolean twoHanded = mainRight ? this.leftArmPose.isTwoHanded() : this.rightArmPose.isTwoHanded();
        ...
        if (this.rightArmPose != HumanoidModel.ArmPose.SPYGLASS) AnimationUtils.bobModelPart(this.rightArm, ageInTicks, 1.0F);
        if (this.leftArmPose != HumanoidModel.ArmPose.SPYGLASS) AnimationUtils.bobModelPart(this.leftArm,  ageInTicks, -1.0F);

        this.animate(entity.teleportGrabAnimation, ScarletLoraAlysiaAnimation.TELEPORT_GRAB, ageInTicks, 1.0F);
    }

    private void setupAttackAnimation(T entity) {
    ...
    public void translateToHand(@NotNull HumanoidArm humanoidArm, @NotNull PoseStack poseStack) {
        (humanoidArm == HumanoidArm.RIGHT ? rightArm : leftArm).translateAndRotate(poseStack);
    }
}
