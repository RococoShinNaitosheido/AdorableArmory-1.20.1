...
void main() {
    ...
    float safeExternalScale = max(externalScale, 0.0001);
    float oneOverExternalScale = 1.0 / safeExternalScale;
    int uvtiles = 16;
    ...
    for (int i = 0; i < 16; i++) {
        ...
        float u = rawu * scale * safeExternalScale;
        float v = (rawv + time * 0.0003420 * oneOverExternalScale) * scale * 0.50 * safeExternalScale;
        ...
        if (symbol >= 0 && symbol < cosmiccount) {
            ...
            float a = tcol.r * (0.5 + (1.0 / float(mult))) * (1.0 - smoothstep(0.20, 0.48, abs(rawv - 0.5)));
            ...
            float hue = mod(time * hueSpeed + float(i) * hueSpread, 1.0);
